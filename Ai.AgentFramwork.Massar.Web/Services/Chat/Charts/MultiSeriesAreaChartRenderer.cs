...
/// <summary>
/// Multi-series area chart (fills under each line). Best for 2-3 series; more will overlap.
/// </summary>
public sealed class MultiSeriesAreaChartRenderer
{
        ...
        int height = 375)
    {
        ...
        // Areas (draw from back to front)
        for (int s = 0; s < seriesValues.Count; s++)
        {
            var color = ChartRenderHelpers.SeriesColor(s, seriesValues.Count);

            using var fillPaint = new SKPaint { IsAntialias = true, Color = new SKColor(color.Red, color.Green, color.Blue, 70), Style = SKPaintStyle.Fill };
