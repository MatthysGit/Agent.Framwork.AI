...
export function openFilePicker(inputFileComponentRef) {
    // Blazor renders <input type="file">; this finds the real DOM element from the component ref
    // We'll grab the first input[type=file] under the component's element.
    // If this doesn't work in your layout, I can adjust it to use an id.
    try {
        const root = inputFileComponentRef?._element || inputFileComponentRef;
        const el = root instanceof HTMLElement ? root : null;
        const input = el?.querySelector?.('input[type="file"]') || document.querySelector('input[type="file"].file-input-hidden');
        input?.click();
    } catch { }
}

window.chatInput = window.chatInput || {};

window.chatInput.openFilePicker = () => {
    const input = document.querySelector('input[type="file"].file-input-hidden');
    input?.click();
};